...
public:
    ...
    Q_INVOKABLE void setRunAfterInstall()
    {
        qDebug() << "setThenrun in.";
        try
        {
            if(QAndroidJniObject::isClassAvailable("com/kaikong/update/thenrun"))
            {
                QAndroidJniObject obj("com/kaikong/update/thenrun");
                QAndroidJniObject activity = QtAndroid::androidActivity();
                if(obj.isValid())
                {
                    obj.callMethod<void>("registerReceiver","(Landroid/app/Activity;)Z",activity.object<jobject>());
                    qDebug()<< "registerReceiver ok.";
                }
                else
                {
                    qDebug()<< "object inVaild.";
                }
            }
            else
            {
                qDebug()<< "com/kaikong/update/thenrun inVaild.";
            }
        }
